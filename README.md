...
- `docs/adr/0025-paypal-payment-provider.md` — PayPal ajouté comme troisième rail (Stripe et Flutterwave se sont révélés indisponibles pour recevoir des paiements en RDC/Maroc, les deux marchés réels du label), calqué sur le modèle Flutterwave (paiement à l'acte, pas d'abonnement natif), Maroc basculé sur PayPal (`countries.default_payment_provider`), **RDC non résolue** (PayPal n'y accepte que l'envoi, pas la réception — reste un problème ouvert), migrations SQL pas encore appliquées au projet Supabase réel (à faire manuellement)
